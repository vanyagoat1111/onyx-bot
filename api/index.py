...
# Разбор по нишам: что должен делать сайт, что меняется, если задача решена,
# и что происходит, если нет. Лежит там же, рядом с ботом.
GUIDE_URL = (os.environ.get("GUIDE_URL", "")
             or (_CHECKLIST_BASE + "/site-guide.html" if _CHECKLIST_BASE else ""))

# ─────────────── Отраслевые гиды ───────────────
#
# Общий разбор по нишам отвечает на вопрос «что должен делать сайт вообще».
# Отраслевой гид отвечает на тот же вопрос внутри одной сферы - и потому
# работает сильнее: человек читает не про сайты, а про свою стройку.
#
# Реестр, а не набор переменных: следующий гид добавляется одной строкой,
# и он автоматически появится и в разделе «Полезное», и на экране примера
# своей ниши. Ключ - код ниши из DEMO_BY_NICHE, чтобы связь не разъезжалась.
NICHE_GUIDES = {
    "construction": {
        "file": "guide-stroy.html",
        "menu": "🏗 Каким должен быть сайт в стройке",
        "what": "Сайт, которому доверяют - гид по стройке и ремонту",
    },
}


def niche_guide(niche):
    """Гид по отрасли или None. Возвращает готовый адрес, а не имя файла:
    вызывающему не нужно знать, где лежит статика."""
    g = NICHE_GUIDES.get(niche or "")
    if not g or not _CHECKLIST_BASE:
        return None
    return dict(g, url=_CHECKLIST_BASE + "/" + g["file"])
...
def render_audit_md(a, p, findings, tid, addons, reasoning):
    ...
    verdict = ("на сайте есть критичные проблемы" if crit else
               # Раньше было «сайт рабочий, но теряет клиентов». Утверждение о потерях,
               # которое мы ничем не подтвердили: аналитики клиента у нас нет.
               "сайт рабочий, но есть что усилить" if findings else "сайт в неплохом состоянии")
...
def render_audit_plan_md(p, findings, addons):
    ...
    md += ["---", "", "# 📌 Как проходит работа", "",
           "| Шаг | Что происходит |", "|---|---|",
           "| 1 | Полчаса разговора: разбираем ваш бизнес и составляем план сайта |",
           "| 2 | Вы отвечаете на вопросы - 10 минут в этом же чате |",
           "| 3 | Мы собираем первую версию и присылаем ссылку |",
           "| 4 | Вы смотрите вживую и говорите, что поправить |",
           "| 5 | Правим, запускаем на вашем домене |", "",
           "---", "",
           "## Почему стоит начать с разговора", "",
           "Отчёт выше собран роботом по открытым данным. Он видит, **что** сломано, "
           "но не знает, откуда к вам приходят клиенты, за что они платят "
           "и чем вы отличаетесь от соседа по улице.", "",
           "На консультации мы это разбираем и решаем, каким должен быть ваш сайт: "
           "что показывать первым, чем доказывать, в каком порядке вести человека "
           "к обращению. Полчаса, бесплатно, без обязательств.", "",
           "**План останется у вас в любом случае** - даже если делать будете "
           "не с нами или сами.", "",
           "> И только после этого мы что-то собираем. Разработка - **0 ₽**: "
           "вы ничего не платите за то, чтобы увидеть результат.", "",
           "Записаться можно прямо сейчас, ближайшее время выберете сами 👇"]
    return "\n".join(md)
...
def audit_plan_kb():
    ...
    Разбор по нишам стоит здесь, а не только в меню, и вот почему.
    Аудит показал человеку список проблем - это момент, когда он согласен,
    что что-то не так, но ещё не понимает, во что это ему обходится.
    Материал отвечает ровно на это: что меняется, если задачу закрыть,
    и что происходит, если нет. Тот, кто не готов записываться сегодня,
    уходит не в пустоту, а в чтение - и возвращается уже с вопросами.

    Ниже консультации и анкеты: горячему человеку нельзя подсовывать
    чтение вперёд разговора."""
    rows = [
    ...
    if GUIDE_URL:
        rows.append([{"text": "🎯 Что должен делать сайт в моей нише", "url": GUIDE_URL}])
...
AUDIT_RULES = [
    ...
    (("ssl", "https", "certificate", "security"), "Проблемы с безопасностью (SSL/HTTPS)",
     "Браузер показывает предупреждение перед входом на сайт - человек видит его раньше, "
     "чем содержимое страницы.",
     ["pages"]),
    (("meta", "title", "description", "h1", "seo", "index", "robots", "sitemap"), "Слабая SEO-основа",
     "Поисковые системы хуже понимают, о чём страница: не хватает заголовков и описаний, "
     "по которым её показывают в выдаче.",
     ["analytics"]),
...
#
# Воронка 4.0 запрещает продавать через выдуманную боль. Нельзя говорить
# «вы теряете клиентов», пока клиент сам это не подтвердил; нельзя обещать
# заявки; нельзя приравнивать поисковые запросы к людям; нельзя считать
# упущенную прибыль без аналитики.
#
# Просить об этом модель в промпте бесполезно: просьбу не проверить.
# Поэтому запреты стоят НА ВЫХОДЕ - текст проверяется перед отправкой,
# и нарушение не доезжает до клиента.
#
# Цена ошибки несимметрична. Лишний раз промолчать - потерять оттенок
# формулировки. Один раз пообещать заявки - получить претензию, на которую
# нечего ответить, потому что обещание отправили мы.

BANNED_CLAIMS = [
    (r"вы\s+тер[яеё]\w*",                          "утверждение о потерях без подтверждения клиента"),
    # Ловим утверждение, а не упоминание. «Вы теряете клиентов» - утверждение,
    # которое мы не доказали. «Как не терять клиента после звонка» - совет,
    # и запрещать его незачем: там нет заявления о бизнесе собеседника.
    (r"(?<!не )тер[яеё]ете\b",                      "утверждение о потерях без подтверждения клиента"),
    (r"тер[яеё]\w+\s+\d",                          "измеримая потеря без данных"),
    (r"упуска\w+\s+[\d\s]*(клиент|заявк|прибыл|деньг)", "утверждение о потерях без данных"),
    (r"гарантиру\w*[^.]{0,30}(заявк|продаж|клиент|трафик|позици|прибыл|рост|результат)",
                                                    "гарантия результата"),
    (r"(обязательно|точно|гарантированно)\s+(получите|вырастет|увеличит|принесёт)",
                                                    "гарантия результата"),
    (r"увеличит\w*[^.]{0,20}(продаж|заявк|прибыл|выручк)\w*\s+(на|в)\s*\d",
                                                    "обещание измеримого роста"),
    (r"конкурент\w*[^.]{0,25}(зарабатыва|получа\w+\s+больше|отбира\w+)",
                                                    "оценка чужой выручки"),
    (r"\d[\d\s]*\s*запрос\w*\s*[=-]\s*\d",          "приравнивание запросов к людям"),
    (r"\d[\d\s]*\s*(человек|клиент\w*)\s+ищ\w+",    "выдача частотности за людей"),
    (r"(потер|недополуч)\w+\s+[\d\s]+\s*(₽|руб)",   "денежная оценка потерь"),
]


def claims_violations(text):
    """Что в тексте нарушает правила достоверности. Пустой список - чисто."""
    if not text:
        return []
    low = " ".join(str(text).lower().split())
    out = []
    for rx, why in BANNED_CLAIMS:
        m = re.search(rx, low)
        if m:
            out.append((m.group(0)[:60], why))
    return out


def claims_guard(text, fallback, where=""):
    """Пропустить текст к клиенту или заменить безопасным вариантом.

    Применяется к тому, что порождено моделью. Наш собственный текст
    проверяется тестами на этапе разработки, а не в бою."""
    bad = claims_violations(text)
    if not bad:
        return text
    try:
        notify_admins("⚠️ <b>Ответ ИИ не прошёл проверку достоверности</b>\n"
                      + (f"Где: {where}\n" if where else "")
                      + "\n".join(f"• «{f}» - {w}" for f, w in bad[:4])
                      + "\n\nКлиенту отправлен безопасный вариант.")
    except Exception as e:
        print("claims notify err", e)
    print("claims blocked:", where, bad[:3])
    return fallback


# Денежную оценку разрешаем только когда есть все четыре величины.
# Без любой из них расчёт превращается в угадывание с видом точности,
# а клиент воспринимает его как обещание.
MONEY_INPUTS = ("analytics_visits", "conversion_rate", "close_rate", "gross_profit_per_sale")


def money_estimate_allowed(data):
    """Можно ли вообще называть деньги. Нужны все четыре исходные величины."""
    d = data or {}
    for k in MONEY_INPUTS:
        v = d.get(k)
        if v in (None, "", 0, "0"):
            return False
        try:
...
def money_missing(data):
    """Чего не хватает для расчёта - чтобы менеджер знал, что спросить."""
    d = data or {}
    return [k for k in MONEY_INPUTS if not d.get(k)]


# Факт без источника в клиентский материал не попадает.
#
# ТЗ требует у каждого факта ссылку и дату проверки. Факт без них - это
# не факт, а наше впечатление, и подавать его клиенту как наблюдение нельзя.
# Такие помечаются и остаются менеджеру на проверку.
def fact_needs_review(fact):
    f = fact or {}
    return not (str(f.get("source") or "").strip() and str(f.get("checked_at") or "").strip())


def facts_for_client(facts):
    """Разделить факты на пригодные клиенту и требующие проверки."""
    ok, review = [], []
    for f in (facts or []):
        (review if fact_needs_review(f) else ok).append(f)
    return ok, review
...
def ai_summarize(url, points_raw, weak, services):
    ...
    prompt = (
        ...
        "ЗАПРЕЩЕНО писать: «вы теряете клиентов», «гарантируем заявки», «конкуренты "
        "зарабатывают больше», приравнивать поисковые запросы к людям и называть "
        "суммы упущенной прибыли. Про последствия можно только вопросом: "
        "«человек может не найти - как это проявляется у вас?»\n\n"
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=25) as r:
            res = json.load(r)
        safe = (f"Мы посмотрели сайт {url_domain(url)}. Это предварительная оценка по открытым "
                f"данным. Основное, что может мешать заявкам: {weak_txt.lower()}. "
                "Точную картину покажет ручная проверка нашей командой.")
        if res.get("content"):  # Anthropic
            out = "".join(b.get("text", "") for b in res["content"] if b.get("type") == "text").strip()
            return claims_guard(out, safe, "резюме аудита")
        if res.get("choices"):  # OpenAI
            out = (res["choices"][0].get("message") or {}).get("content", "").strip()
            return claims_guard(out, safe, "резюме аудита")
    except Exception as e:
        print("AI err:", e)
...
WELCOME = (
    "👋 <b>ONYX WEB</b>\n"
    "<i>Сайты для бизнеса · сначала показываем, потом счёт</i>\n\n"
...
TARIFFS_INFO = (
    ...
    "Сначала собираем сайт и показываем. Платите только за запуск и только после того, как утвердите результат.\n\n"
    "• Разработка - <b>0 ₽</b>\n"
    "• Запуск под ключ - <b>от 8 990 ₽</b>\n"
...
MAIN_MENU = {"keyboard": [
    ...
    # «Полезное» стоит третьей строкой, а не в самом низу рядом с «Документы».
    # Это материалы для человека, который ещё выбирает, - то есть для
...
def demos_kb(niche):
    rows = [[{"text": f"🔗 Открыть {d['name']}", "url": demo_link(d)}]
            for _, d in demos_for(niche)]
    # Отраслевой гид ровно здесь и нужен: человек только что посмотрел
    # готовый сайт своей сферы и думает «а мне такое надо?». Гид отвечает
    # именно на это - и отвечает про его отрасль, а не про сайты вообще.
    # Ниже ссылки на демо, но выше «продолжить»: не готовый к вопросам
    # человек уходит в чтение, а не из бота.
    g = niche_guide(niche)
    if g:
        rows.append([{"text": g["menu"], "url": g["url"]}])
...
# ─────────────────── Полезные материалы ───────────────────
#
# Два документа и витрина, собранные в одном месте. Порядок здесь не
# алфавитный, а по готовности человека к покупке - от дальнего к близкому:
#
#   1. «Что должен делать сайт» - для того, кто ещё не понимает, зачем
#      ему сайт и что он должен делать. Самый дальний от покупки.
#   2. «Как заказать сайт и не пожалеть» - для того, кто уже решил делать
#      и теперь выбирает подрядчика. Здесь он сравнивает.
#   3. Примеры сайтов - для того, кто выбирает, как это будет выглядеть.
#      Ближе всех к заявке.
#
# Ставить наоборот бессмысленно: человек, который ещё не понял, зачем ему
# сайт, не станет читать про выбор подрядчика.

USEFUL_MD = (
    "# 📚 Полезное\n\n"
    "Три материала, которые помогают разобраться, даже если работать "
    "будете не с нами.\n\n"
    "### Что должен делать сайт в вашем бизнесе\n"
    "Разбор по 14 отраслям: главное сомнение, с которым к вам приходит "
    "клиент, что это сомнение снимает, и что происходит, если не снять. "
    "Читать 10 минут.\n\n"
    "### Как заказать сайт и не пожалеть\n"
    "Чек-лист: что собрать до старта, как проверить студию, что смотреть "
    "в договоре и по каким признакам подрядчика лучше обойти. Галочки "
    "живые, отметки сохраняются - можно заполнять постепенно.\n\n"
    "### Гид по вашей отрасли\n"
    "Разбор внутри одной сферы: слабые и сильные формулировки рядом, "
    "экспресс-диагностика с оценкой и итоговый чек-лист. Сейчас готов гид "
    "по стройке и ремонту.\n\n"
    "### Примеры готовых сайтов\n"
    "14 живых страниц по отраслям. Не картинки: можно листать, кликать "
    "меню и заполнять формы.\n\n"
    "> Материалы бесплатны и ни к чему не обязывают. Ссылку можно "
    "переслать - партнёру, бухгалтеру, кому угодно."
)


def useful_kb(back="b:home", niche=None):
    """Кнопки раздела «Полезное».

    Кнопка появляется только если адрес материала реально известен: пустая
    ссылка в Telegram даёт ошибку на весь запрос, и вместо раздела человек
    увидит, что бот сломался.

    Если ниша клиента известна из анкеты и по ней есть отраслевой гид -
    он идёт ПЕРВЫМ. Материал про его сферу всегда сильнее общего: человек
    читает не про сайты, а про свою стройку.

    Если ниша не известна, отраслевые гиды всё равно показываем списком.
    Их пока мало, и владелец стройки сам себя узнает; когда станет больше
    десятка, здесь понадобится отдельный экран выбора отрасли."""
    rows = []
    own = niche_guide(niche)
    if own:
        rows.append([{"text": own["menu"], "url": own["url"]}])
    if GUIDE_URL:
        rows.append([{"text": "🎯 Что должен делать сайт", "url": GUIDE_URL}])
    if CHECKLIST_URL:
        rows.append([{"text": "📋 Как заказать сайт и не пожалеть", "url": CHECKLIST_URL}])
    if not own:
        for key in NICHE_GUIDES:
            g = niche_guide(key)
            if g:
                rows.append([{"text": g["menu"], "url": g["url"]}])
    rows.append([{"text": "🖼 Примеры сайтов по отраслям", "callback_data": "useful:gal"}])
    rows.append([{"text": "🔍 Разобрать мой сайт - бесплатно", "callback_data": "useful:audit"}])
    rows.append(help_row(back))
    return {"inline_keyboard": rows}


def useful_niche(uid):
    """Ниша клиента, если он проходил анкету. Молча отдаём None при любой
    неожиданности: раздел «Полезное» не должен падать из-за профиля."""
    try:
        return (user_get(uid) or {}).get("niche") or None
    except Exception as e:
...
def send_useful(chat_id, uid=None):
    niche = useful_niche(uid) if uid else None
    if uid:
        try:
            log_event(uid, "useful_open")
        except Exception as e:
            print("useful log err", e)
    send_rich(chat_id, USEFUL_MD, reply_markup=useful_kb(niche=niche))
...
ENTRY_MD = (
    ...
    "Мы соберём сайт целиком, покажем вживую, и только если он вам "
    "понравится - вы оплатите запуск. Не понравится: разойдёмся, "
    "вы не должны ни рубля.\n\n"
    "Сама разработка при этом бесплатна. Не акция - так устроена наша работа.\n\n"
...
# ═══════════════════ РЕЖИМ КЛИЕНТА ═══════════════════
#
# Воронка 4.0 разворачивает приоритет: основной клиент - действующий бизнес
# БЕЗ сайта. Раньше «сайта нет» было просто кнопкой, которая выдавала
# чек-лист почитать. Теперь это режим, от которого зависит весь дальнейший
# путь: какой аудит запускать, что писать в карточку менеджеру, какие
# статусы вести и каким языком разговаривать.
#
# Третье состояние - UNCLEAR - не формальность. Человек присылает адрес,
# и мы не знаем, его ли это сайт: бывает страница на конструкторе от бывшего
# подрядчика, поддомен агрегатора, карточка на маркетплейсе или сайт
# однофамильца. Разбирать такой адрес как «ваш сайт» - значит строить
# разговор на непроверенном основании. Пока владение не подтверждено,
# аудит не идёт.

SITE_NO, SITE_HAS, SITE_UNCLEAR = "NO_SITE", "HAS_SITE", "UNCLEAR"

SITE_MODE_RU = {
    SITE_NO: "сайта нет",
    SITE_HAS: "сайт есть",
    SITE_UNCLEAR: "не подтверждён",
}

# Адреса, которые человек называет «моим сайтом», но сайтом бизнеса они
# не являются: это арендованные площадки. Их владелец - площадка, а не он.
# Для воронки такой клиент - NO_SITE: собственного центра доверия у него нет.
RENTED_HOSTS = (
    "vk.com", "vk.ru", "t.me", "telegram.me", "instagram.com", "facebook.com",
    "ok.ru", "yandex.ru/maps", "yandex.ru/profile", "google.com/maps",
    "2gis.ru", "avito.ru", "ozon.ru", "wildberries.ru", "youla.ru",
    "zoon.ru", "flamp.ru", "prodoctorov.ru", "profi.ru", "yell.ru",
    "taplink.cc", "linktr.ee", "wa.me",
)


def rented_host(url):
    """Это арендованная площадка, а не собственный сайт бизнеса."""
    low = (url or "").lower()
    return any(h in low for h in RENTED_HOSTS)


def site_mode_of(uid):
    """Режим клиента. По умолчанию - неизвестен, а не «сайта нет»."""
    return (user_get(uid) or {}).get("site_mode", "")


def site_mode_set(uid, mode, url="", owner_confirmed=None, why=""):
    """Записать режим. Пишется и в профиль, и в таблицу - менеджеру он нужен
    раньше, чем что-либо ещё: от него зависит сценарий звонка."""
    if mode not in SITE_MODE_RU:
        return None
    p = user_get(uid) or {}
    p["site_mode"] = mode
    p["site_mode_why"] = (why or "")[:200]
    if url:
        p["website_url"] = url
    if owner_confirmed is not None:
        p["website_owner_confirmed"] = bool(owner_confirmed)
    user_save(uid, p)
    try:
        post_to_sheet({"table": "Clients", "telegram_id": uid,
                       "site_mode": mode,
                       "site_mode_ru": SITE_MODE_RU[mode],
                       "website_url": p.get("website_url", ""),
                       "website_owner_confirmed": "да" if p.get("website_owner_confirmed") else "нет",
                       "updated_at": now_str()})
    except Exception as e:
        print("site_mode sheet err", e)
    return p


def site_mode_ready(uid):
    """Можно ли идти дальше по воронке.

    UNCLEAR - это стоп. ТЗ прямо запрещает продолжать аудит, пока адрес
    или владение не подтверждены."""
    return site_mode_of(uid) in (SITE_NO, SITE_HAS)


def classify_url(url):
    """Что делать с присланным адресом: (режим, причина).

    Разбирать сам адрес мы умеем, а вот чей он - нет. Поэтому арендованные
    площадки отправляем в NO_SITE сразу (там владение и не подразумевается),
    а обычный домен - в UNCLEAR до подтверждения человеком."""
    if not url:
        return SITE_UNCLEAR, "адрес не распознан"
    if rented_host(url):
        return SITE_NO, "прислана страница на чужой площадке, а не собственный сайт"
    return SITE_UNCLEAR, "нужно подтверждение, что сайт принадлежит бизнесу"


OWNER_ASK_MD = (
    "# 🔐 Один вопрос перед разбором\n\n"
    "Этот сайт принадлежит вашему бизнесу?\n\n"
    "Спрашиваю не из формальности. Разбор строится на предположении, "
    "что менять сайт можно. Если домен оформлен на бывшего подрядчика "
    "или это страница на чужой площадке, половина рекомендаций окажется "
    "невыполнимой, а разговор - бесполезным."
)


def owner_kb(url):
    return {"inline_keyboard": [
        [{"text": "✅ Да, сайт наш", "callback_data": "own:yes"}],
        [{"text": "🤷 Не уверен / оформлен на подрядчика", "callback_data": "own:unsure"}],
        [{"text": "❌ Это не наш сайт", "callback_data": "own:no"}],
    ]}


def entry_kb():
    # Порядок кнопок - это заявление о том, кого мы ждём.
    #
    # Раньше первой стояла «сайт есть». По воронке 4.0 основной клиент -
    # действующий бизнес БЕЗ сайта, и ставить его вторым значит на входе
    # сообщать ему, что он тут не главный.
    return {"inline_keyboard": [
        [{"text": "📋 Сайта нет - с чего начать", "callback_data": "entry:nosite"}],
        [{"text": "🌐 Сайт есть - разберите его", "callback_data": "entry:site"}],
        [{"text": "🤝 Я уже общался с менеджером", "callback_data": "entry:manager"}],
    ]}


# Ветка «сайта нет» ведёт на разговор, а не в читальный зал.
#
# Раньше первым делом человек получал гайд и чек-лист, а встреча стояла
# третьим пунктом. Материалы полезные, но это отложенное действие: их
# сохраняют и не возвращаются. По воронке 4.0 этот человек - основной
# клиент, и ему нужен не файл, а десять минут разговора.
#
# Формулировка про последствия - вопросом, а не утверждением. Мы не знаем,
# теряет ли он клиентов, и говорить это за него нельзя.
NOSITE_MD = (
    "# 📋 Тогда начнём с главного\n\n"
    "Сайта нет - значит чинить нечего, надо понять, каким он должен быть "
    "именно у вас.\n\n"
    "### 10 минут: путь вашего покупателя\n"
    "Пройдём его глазами человека, который вас не знает. Что он находит по "
    "названию, что понимает про услугу без вашего объяснения, где сравнивает "
    "вас с другими и на чём останавливается.\n\n"
    "В конце - структура вашего первого сайта и диапазон цены. "
    "Бесплатно и ни к чему не обязывает.\n\n"
    "### Если хотите сначала почитать\n"
    "Разбор по вашей отрасли: что должен делать сайт в вашей нише и с каким "
    "сомнением к вам приходит клиент. И чек-лист: что собрать до начала "
    "работы и о чём договориться с подрядчиком, чтобы потом не переделывать. "
    "Полезны, даже если работать будете не с нами.\n\n"
...
def nosite_kb():
    # Первой кнопкой - действие, остальное поддержкой.
    #
    # Было наоборот: сначала два материала, разговор третьим. Ссылка на файл
    # выигрывает у кнопки разговора почти всегда - её проще нажать и легче
    # отложить. В итоге основной клиент уходил читать вместо того, чтобы
    # назначить встречу.
    rows = [[{"text": "🎯 Разобрать мой случай за 10 минут", "callback_data": "cons:offer"}]]
    if GUIDE_URL:
        rows.append([{"text": "📗 Каким должен быть сайт в моей нише", "url": GUIDE_URL}])
    if CHECKLIST_URL:
        rows.append([{"text": "📖 Чек-лист по подготовке", "url": CHECKLIST_URL}])
...
CONSULT_MD = (
    ...
    "---\n\n"
    "### Что вы получите\n"
    "- Разбор текущей ситуации: откуда идут клиенты и что происходит на пути к заявке\n"
...
# Партнёрская программа. По стратегии больше половины запусков должны идти
# через партнёров, а главный партнёр - не фрилансер с одним знакомым, а веб- или
# smm-студия с потоком заказов. Ей нужна не «комиссия за рекомендацию», а
# производственная мощность под своим брендом и гарантия, что мы не заберём
# её клиента. Поэтому раздел говорит на языке студии, а реферальная схема
# оставлена как второй, более лёгкий вариант входа.
PARTNER_TEXT = (
    "🤝 <b>Партнёрам: производство под вашим брендом</b>\n\n"
    "Вы берёте заказы, мы их делаем. Клиент остаётся вашим - мы с ним "
    "не связываемся и ничего ему не предлагаем.\n\n"
    "<b>Что получает студия</b>\n"
    "- Оптовую цену: ваша маржа внутри вашей же сметы\n"
    "- Первую версию сайта за 2-3 дня, без найма и простоя\n"
    "- Работу под вашим брендом: клиент видит вас, а не нас\n"
    "- Возможность брать заказы, которые раньше отдавали конкурентам\n\n"
    "<b>Что получает специалист</b>\n"
    "Если своей команды нет - просто приводите клиента и получаете "
    "вознаграждение с каждого запуска. Всё общение и работу берём на себя.\n\n"
    "<b>Кому это подходит</b>\n"
    "Веб-студиям с очередью, SMM- и performance-агентствам, у чьих клиентов "
    "нет нормального сайта, дизайнерам и маркетологам без своей разработки.\n\n"
    "<i>Правило простое: ваш клиент - ваш. Никаких прямых предложений от нас.</i>"
)

PARTNER_HOW = (
    "ℹ️ <b>Как устроена работа</b>\n\n"
    "<b>Со студией</b>\n"
    "1. Присылаете заказ - бриф, сроки, что нужно\n"
    "2. Мы подтверждаем срок и оптовую цену\n"
    "3. Собираем сайт и отдаём вам на проверку\n"
    "4. Вы показываете клиенту как свою работу\n"
    "5. Правки - через вас, мы с клиентом не пересекаемся\n\n"
    "<b>С рекомендацией</b>\n"
    "1. Приводите клиента\n"
    "2. Мы фиксируем, что он от вас\n"
    "3. Клиент оплачивает запуск\n"
    "4. Вы получаете вознаграждение\n\n"
    "Условия и цены обсуждаем лично: они зависят от объёма. "
    "Оставьте заявку - вернёмся в течение 30 минут."
)
...
PARTNER_STEPS = [
    ("name", "Как вас зовут?"),
    ("ptype", "Кто вы: студия с командой или специалист без своей разработки?"),
    ("activity", "Чем занимаетесь? (веб-студия, SMM, таргет, дизайн, консалтинг)"),
    ("volume", "Сколько сайтов в месяц вы могли бы отдавать? Примерно, навскидку."),
    ("contact", "Ваш Telegram для связи (например: @username)"),
    ("portfolio_link", "Ссылка на сайт, соцсети или портфолио:"),
    ("comment", "Что для вас важнее всего в подрядчике? (если нечего добавить - «нет»)"),
]
...
DESIGN_MODE_RULES = {
    # Оба режима обязательно проходят дизайн-ядро. Без него модель собирает
    # середину распределения - тёмный фон, фиолетовый градиент, Inter, три
    # карточки по центру - и сайт читается как «сделала нейросеть».
    "STANDARD": "РЕЖИМ ДИЗАЙНА: STANDARD.\n"
                "Базовый дизайн по стандартам выбранного тарифа. "
                "Опция «Индивидуальный дизайн» не оплачена - не обещать уникальную концепцию.\n"
                "ОБЯЗАТЕЛЬНО: перед сборкой прогнать через скилл onyx-design "
                "(арт-направление выбирается до кода, один запоминающийся приём, "
                "стоп-лист признаков нейросети, проверка по скриншоту).",
    "INDIVIDUAL_CLAUDE": "РЕЖИМ ДИЗАЙНА: INDIVIDUAL_CLAUDE.\n"
                         ...
                         "ОБЯЗАТЕЛЬНО: сначала скилл onyx-design (база), затем скилл "
                         "onyx-design-pro (разбор своей работы, авторская графика и типографика, "
                         "углублённый интерактив, приёмка по скриншотам). "
                         "Второй скилл не заменяет первый.",
}
...
def factory_client_data_block(uid, d):
    ...
    rows = [
        ...
        # Ниша попадает в анкету на старте, но если человек вернулся к старой
        # незаконченной анкете, поля может не быть. Тогда берём из профиля:
        # без ниши первый промпт соберёт обезличенный сайт.
        ("НИША", d.get("niche") or NICHE_RU.get(p.get("client_niche", ""), "").split(" ", 1)[-1]),
        ("ГОРОД", d.get("city", "")),
...
# Пакеты запуска. Разработка в любом - 0 ₽, оплачивается только запуск.
# Кроме состава у каждого пакета есть:
#   result  - что человек получит на практике, а не список работ;
#   pages   - объём, чтобы не сравнивали одностраничник с многостраничником;
#   days    - срок до первой версии;
#   not_inc - чего в пакете НЕТ. Названное честно ограничение снимает
#             больше возражений, чем ещё один пункт «что входит»:
#             человек перестаёт искать подвох и спокойно доплачивает за нужное.
TARIFFS = [
    {"id": "start", "name": "Старт онлайн", "price": 8990,
     "desc": "Быстрый выход в интернет: одна страница, которая объясняет, "
             "чем вы занимаетесь, и даёт способ с вами связаться.",
     "result": "Через 2-3 дня у вас есть адрес, который не стыдно дать клиенту, "
               "отправить в переписке и поставить в шапку профиля.",
     "pages": "Одна страница", "days": "2-3 дня",
     ...
     "includes": ["Одностраничный сайт под ваш бизнес",
                  "Адаптация под телефон, планшет и компьютер",
                  "Первый экран с оффером: чем вы полезны и кому",
                  "Блок услуг или товаров с описанием",
                  "Блок «О компании» и преимущества",
                  "Контакты, карта проезда и кнопки мессенджеров",
                  "Форма заявки с проверкой полей",
                  "Заголовки и описания для поисковиков",
                  "Домен, хостинг и SSL-сертификат",
                  "Публикация и техническая настройка"],
     "not_inc": ["аналитики и целей", "CRM и онлайн-записи", "каталога с фильтрами"]},

    {"id": "leads", "name": "Сайт + заявки", "price": 13990, "best": True,
     "desc": "Сайт, задача которого - обращения. Структура выстроена так, "
             "чтобы человек дошёл до формы, а вы узнали об этом сразу.",
     "result": "Заявки падают вам в Telegram за секунду, а в Метрике видно, "
               "откуда пришёл каждый обратившийся и что он смотрел.",
     "pages": "Одна длинная страница", "days": "3-4 дня",
     ...
     "includes": ["Всё из пакета «Старт онлайн»",
                  "Структура под заявку: путь от первого экрана до формы",
                  "Проработанный оффер и заголовки под вашу нишу",
                  "Блок доверия: отзывы, гарантии, кейсы или цифры",
                  "Блок вопросов и ответов, который снимает возражения",
                  "Формы на каждом экране, а не одна внизу",
                  "Уведомления о заявках в Telegram",
                  "Яндекс Метрика с настроенными целями",
                  "Карта или геосервис с маршрутом",
                  "Проверка форм с реальных устройств перед запуском"],
     "not_inc": ["CRM-интеграции", "онлайн-записи и калькулятора", "каталога товаров"]},

    {"id": "system", "name": "Сайт как система продаж", "price": 19990, "approx": True,
     "desc": "Сайт, заявки, аналитика и контроль обработки в одном контуре. "
             "Для тех, кто платит за рекламу и не готов терять обращения.",
     "result": "Заявка попадает в CRM или таблицу, не теряется и не забывается, "
               "а по каждому источнику видно, окупается он или сливает бюджет.",
     "pages": "Несколько страниц", "days": "5-7 дней",
     ...
     "includes": ["Всё из пакета «Сайт + заявки»",
                  "Многостраничная структура: услуги, кейсы, о компании",
                  "Отдельная страница под каждое ключевое направление",
                  "Дополнительные продающие блоки под вашу нишу",
                  "CRM-интеграция или таблица заявок с уведомлениями",
                  "Онлайн-запись или калькулятор стоимости - на выбор",
                  "Расширенная аналитика: цели, события, источники",
                  "Разметка для рекламных кабинетов",
                  "Подготовка посадочных под рекламные кампании",
                  "Инструкция, как читать статистику и что с ней делать"],
     "not_inc": ["интернет-магазина с корзиной и оплатой", "ведения рекламы", "производства контента"]},
]
...
def tariffs_list_text(uid):
    """Сводка пакетов. Раньше был только заголовок и одна строка описания -
    человек не понимал, чем пакеты отличаются, и не мог выбрать. Добавили
    объём, срок и главную выгоду: этого хватает, чтобы сориентироваться,
    не открывая каждую карточку."""
    chosen = (user_get(uid) or {}).get("chosen_tariff")
    head = ["📦 <b>Пакеты запуска</b>", "",
            "Разработка в любом - <b>0 ₽</b>. Платите только за запуск "
            "и только после того, как утвердите готовый сайт.", ""]
    for t in TARIFFS:
        mark = "✅ " if chosen == t["id"] else ""
        best = "  ⭐️ <i>берут чаще всего</i>" if t.get("best") else ""
        head.append(f"{mark}<b>{t['name']}</b> · {tariff_price(t)}{best}")
        head.append(f"<i>{t.get('pages', '')} · первая версия за {t.get('days', '')}</i>")
        head.append(t["desc"])
        head.append("")
    head.append("Нажмите на пакет - покажем состав, сроки и что в него не входит 👇")
    return "\n".join(head)
...
def tariff_card_text(tid, uid):
    """Развёрнутая карточка пакета.

    Порядок не случайный: сначала что человек получит на практике, потом
    сроки и объём, потом состав, и только в конце - чего в пакете нет.
    Названное ограничение снимает больше возражений, чем ещё один пункт
    «что входит»: человек перестаёт искать подвох."""
    t = TARIFF.get(tid)
    if not t:
        return "Пакет не найден."
    lines = [f"📦 <b>{t['name']}</b>", f"<i>{t['desc']}</i>", ""]

    if t.get("result"):
        lines += [f"<b>Что вы получите.</b> {t['result']}", ""]

    lines += ["<b>Коротко</b>",
              f"• Объём: {t.get('pages', 'одна страница')}",
              f"• Первая версия: {t.get('days', '2-3 дня')}",
              f"• Разработка: <b>0 ₽</b>",
              f"• Запуск: <b>{tariff_price(t)}</b> - платите после того, как утвердите сайт",
              "", "<b>Что входит</b>"]
    lines += [f"✓ {x}" for x in t["includes"]]

    if t.get("not_inc"):
        lines += ["", "<b>Чего в пакете нет</b>",
                  "✗ " + ", ".join(t["not_inc"]),
                  "<i>Это можно добавить отдельно - скажем цену до начала работ, "
                  "без сюрпризов по ходу.</i>"]

    lines += ["", f"<b>Кому подходит</b>\n{t['for']}"]
    if (user_get(uid) or {}).get("chosen_tariff") == tid:
        lines.append("\n✅ <i>Этот пакет выбран в вашей заявке.</i>")
    return "\n".join(lines)
...
def process_message(msg):
    ...
    if is_admin(uid) and text.startswith("/"):
        ...
        if low.startswith("/richtest"):
            ...
            demo = ("## ✅ Rich Messages работают\n\n"
                    ...
                    "| Старт онлайн | 8 990 ₽ | **0 ₽** |\n"
                    "| Сайт + заявки | 13 990 ₽ | **0 ₽** |\n"
                    "| Система продаж | от 19 990 ₽ | **0 ₽** |\n\n"
    ...
    if st and st.get("flow") == "audit_url":
        ...
        # Чей это сайт - мы по адресу не узнаем, а строить на этом разговор нельзя.
        режим, почему = classify_url(url)

        if режим == SITE_NO:
            # Прислали страницу на чужой площадке. Разбирать её как «ваш сайт»
            # неверно: менять там нечего, владелец - площадка.
            state_del(uid)
            site_mode_set(uid, SITE_NO, url=url, owner_confirmed=False, why=почему)
            crm_set(uid, "material_sent", notify=False)
            send_rich(chat_id,
                      "# Это страница на чужой площадке\n\n"
                      "Разбирать её как сайт бессмысленно: оформление, структура и правила "
                      "там задаются площадкой, а не вами. Уйдёт площадка или изменит правила - "
                      "уйдёт и страница.\n\n"
                      "Разберу другое: каким должен быть ваш собственный сайт. "
                      "Это займёт 10 минут.",
                      reply_markup=nosite_kb())
            schedule_followup(uid, "consult_invite", user.get("username", ""))
            return

        if режим == SITE_UNCLEAR and not (user_get(uid) or {}).get("website_owner_confirmed"):
            state_set(uid, {"flow": "audit_url", "pending_url": url})
            site_mode_set(uid, SITE_UNCLEAR, url=url, why=почему)
            send_rich(chat_id, OWNER_ASK_MD, reply_markup=owner_kb(url))
            return
...
def process_callback(cq):
    ...
    # Раздел «Полезное». Заходы отсюда помечаем отдельно от прямых:
    # человек, пришедший в витрину из материала, уже прочитал про свою нишу
    # и разговаривать с ним надо иначе, чем с тем, кто просто листает меню.
    if data == "useful:open":
        answer_cb(cq["id"])
        edit_rich(chat_id, mid, USEFUL_MD, None, useful_kb()); return
    if data == "useful:gal":
        answer_cb(cq["id"])
        log_event(uid, "gallery_open")
        lead_touch(uid, username=user.get("username"), action="gallery_from_useful")
        edit_rich(chat_id, mid, gallery_md(), None, gallery_kb()); return
    if data == "useful:audit":
        answer_cb(cq["id"])
        state_set(uid, {"flow": "audit_url"})
        send(chat_id, AUDIT_INTRO, {"keyboard": [[{"text": "🏠 Главное меню"}]],
                                    "resize_keyboard": True}); return
    ...
    if data == "entry:nosite":
        answer_cb(cq["id"])
        site_mode_set(uid, SITE_NO, why="клиент сам выбрал ветку «сайта нет»")
        crm_set(uid, "material_sent", notify=False)
    ...
    if data.startswith("own:"):
        answer_cb(cq["id"])
        st_own = state_get(uid) or {}
        url = st_own.get("pending_url") or (user_get(uid) or {}).get("website_url", "")
        выбор = data.split(":", 1)[1]

        if выбор == "yes":
            state_del(uid)
            site_mode_set(uid, SITE_HAS, url=url, owner_confirmed=True,
                          why="клиент подтвердил владение")
            log_event(uid, "site_owner_confirmed")
            if not rate_ok(uid, "audit"):
                rate_notice(chat_id, uid, "audit"); return
            audit_start(chat_id, uid, url, username=user.get("username", ""))
            return

        if выбор == "no":
            state_del(uid)
            site_mode_set(uid, SITE_NO, url="", owner_confirmed=False,
                          why="клиент сказал, что сайт не его")
            crm_set(uid, "material_sent", notify=False)
            edit_rich(chat_id, mid,
                      "# Тогда начнём с чистого листа\n\n"
                      "Разбирать чужой сайт смысла нет. Посмотрим, каким должен быть ваш: "
                      "что на нём нужно вашей нише и с чего начать.",
                      reply_markup=nosite_kb())
            schedule_followup(uid, "consult_invite", user.get("username", ""))
            return

        # «Не уверен» - это не отказ, а самый частый случай у малого бизнеса.
        # Дальше по воронке не пускаем, но и не бросаем: подключаем человека.
        state_del(uid)
        site_mode_set(uid, SITE_UNCLEAR, url=url, owner_confirmed=False,
                      why="владение не подтверждено, нужен менеджер")
        log_event(uid, "site_owner_unclear")
        escalate(uid, "Не подтверждено владение сайтом", f"Адрес: {url or 'не назван'}")
        edit_rich(chat_id, mid,
                  "# Разберёмся вместе\n\n"
                  "Это обычная история: домен часто оформлен на того, кто когда-то делал сайт. "
                  f"Передал {MANAGER_NAME} - он поможет проверить, на кого зарегистрирован домен "
                  "и как вернуть его себе. Это бесплатно и ни к чему не обязывает.\n\n"
                  "А пока можно посмотреть, каким должен быть сайт в вашей нише.",
                  reply_markup=nosite_kb())
        return
