...
TARIFFS = [
    dict(key="start", num="01", level=1, accent=(0, 168, 252),
         ...
         inc=["Одностраничный сайт под ключ", "Адаптация: телефон, планшет, ПК",
              "Первый экран с оффером", "Блок услуг и о компании",
              "Форма заявки и кнопки связи", "Домен · хостинг · SSL",
    ...
    dict(key="leads", num="02", level=2, accent=(96, 116, 250),
         ...
         inc=["Сайт под ключ с адаптацией", "Усиленная структура под заявки",
              "Блок доверия: отзывы, гарантии", "Блок FAQ — снимает возражения",
              "Telegram-уведомления о заявках", "Яндекс Метрика и цели",
              "Домен · хостинг · SSL"],
    ...
    dict(key="system", num="03", level=3, accent=(197, 84, 252),
         ...
         inc=["Многостраничный сайт под ключ", "Структура под заявки + FAQ",
              "CRM или таблица заявок", "Онлайн-запись или калькулятор",
              "Расширенная аналитика и цели", "Подготовка к рекламе",
              "Домен · хостинг · SSL"],
         who="", badge=None),
]
...
def compose(t, cfg, W=720, H=900):
    ...
    for it in cfg["inc"]:
        ...
        d.text((px(68), y), it, font=F("Regular", px(21)), fill=(214, 219, 228))
        y += px(29)
